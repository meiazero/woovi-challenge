...
export function OptionCard({
  ...
  return (
    <Box
      className={cn(
        "rounded-[10px] border-2 border-zinc-200",
        checked && "selected",
        className,
      )}
      onClick={onSelect}
      ...
      <div className="p-5">
        <div className="flex flex-row items-center justify-between">
          <p className="text-2xl font-semibold">
            <span className="font-extrabold">{card.parcela}</span> R${" "}
            {card.valor}
          </p>
          {checked ? (
            <CircleCheckIcon className="size-6 fill-primary text-zinc-50" />
          ) : (
        ...
        {card.discount ? (
          <p className="text-primary text-base">{card.discount}</p>
        ) : (
        ...
        {card.banner && (
          <div className="pt-1 relative">
            <Flag className="text-secondary w-full h-auto" />
